...
import '../model/store_banner_model.dart';
...
class StoreRepository {
  ...
  // TODO: Make Trending Stores
  Future<Either<Failure, List<StoreBannerModel>>> getTrendingStores({int limit = 5}) async {
    return safeSupabaseCall(
      () async {
        final response = await supabaseClient.from('stores').select().limit(limit).order('created_at', ascending: false);
        return (response as List).map((e) => StoreBannerModel.fromJson(e)).toList();
      },
    );
  }
}
